...
export function KniffligeStellen({ className }: { className?: string }) {
  ...
  return (
    <Karte akzent="creme" className={`px-5 py-3 ${className ?? ""}`}>
      <div className="flex items-center gap-3">
        ...
        <ul className="flex min-w-0 flex-1 flex-wrap gap-1.5">
          {liste.map(({ schluessel, eintrag }) => (
            <li
              ...
              className="rounded-full bg-creme/70 px-3 py-1 text-sm font-semibold text-tinte"
            >
              {eintrag.bezeichnung}
            </li>
          ))}
        ...
        <button
          type="button"
          onClick={vergessen}
          className="shrink-0 rounded-full px-3 py-1 text-xs text-tinte-leise transition-colors hover:bg-papier-tief"
        >
          zurücksetzen
        </button>
      </div>
